...
Manipulator::Manipulator(shared_ptr<DERObject> obj) {
    this->derobj = obj;
    ...
    if (!initialized) {
        ...
        for (; c_start < c_end; c_start++) {
            *(c+c_start) = rand() % 254 + 1; // avoid null terminator
        }
...
shared_ptr<Manipulator> Manipulator::make_manipulator(shared_ptr<DERObject> obj) {
    ...
    switch (int(obj->raw_tag)) {
        case ASN1_TYPE_INTEGER:
            r = shared_ptr<Manipulator>(new IntManipulator(obj));
            break;

        case ASN1_TYPE_OBJECT_IDENTIFIER:
            r = shared_ptr<Manipulator>(new OIDManipulator(obj));
            break;

        case ASN1_TYPE_PRINTABLESTRING:
            r = shared_ptr<Manipulator>(new PrintableStringManipulator(obj));
            break;

        case ASN1_TYPE_UTCTime:
            r = shared_ptr<Manipulator>(new UTCTimeManipulator(obj));
            break;

        case ASN1_TYPE_IA5STRING:
            r = shared_ptr<Manipulator>(new IA5StringManipulator(obj));
            break;

        case ASN1_TYPE_UTF8STRING:
            r = shared_ptr<Manipulator>(new UTF8StringManipulator(obj));
            break;

        case ASN1_TYPE_GeneralizedTime:
            r = shared_ptr<Manipulator>(new GeneralizedTimeManipulator(obj));
            break;
    }
...
*/
string Manipulator::general_random_string_manipulation() {
    Botan::RandomNumberGenerator *rng = Botan::RandomNumberGenerator::make_rng();
    Botan::BigInt r;
    Botan::BigInt min_length = 10;
    Botan::BigInt max_length = 1000;
    Botan::BigInt length = r.random_integer(*rng, min_length, max_length);

    vector<string> charPool = {"a", "A", "b", "B", "c", "C", "d", "D", "e", "E", "f", "F", "g", "G", "h", "H", "i", "I", "j", "J", "k", "K", "l", "L", "m", "M", "n", "N", "o",
                                "O", "p", "P", "q", "Q", "r", "R", "s", "S", "t", "T", "u", "U", "v", "V", "w", "W", "x", "X", "y", "Y", "z", "Z", "1", "2", "3", "4", "5", "6",
                                "7", "8", "9", "ä", "ö", "ü", "!", "\"", "'", "§", "$", "%", "&", "/", "(", ")", "=", "?", "ß", "*", "+", "#", "-", ".", ":", "_", ",", ";",
                                "%s", "%f", "%d", "%i", "%u", "%o", "%e", "%c", "%a", "%p", "%n", "%g", "%G", "%F", "%A", "%x", "%X"
    ...
    for (int i=0; i < length; i++) {
        s += charPool[r.random_integer(*rng, 0, charPool.size()-1).to_u32bit()];
    }
    return s;
}
