...
IntManipulator::IntManipulator(shared_ptr<DERObject> obj) : Manipulator(obj) {
    set_fixed_manipulations();
}
...
void IntManipulator::set_fixed_manipulations() {
...
Botan::BigInt IntManipulator::get_value() {
...
void IntManipulator::generate(bool random, int index) {
    if (!random) {
        if (index == -1)
        ...
        // create random number X (number of bits) and then create a random number of X bit

        Botan::BigInt x;
        x = x.random_integer (*Botan::RandomNumberGenerator::make_rng(), 1, 4048);

        Botan::BigInt random_number;
        random_number.randomize(*Botan::RandomNumberGenerator::make_rng(), x.to_u32bit(), true);

        this->set_value(random_number);
    }
}
